...
main() {
    ...
    # Directly PATCH every finding in the engagement with its EPSS score and
    # percentile via the DefectDojo API.  This is the only reliable method —
    # DefectDojo's parsers do not read the EPSS field from scan JSON files.
    if command -v python3 &>/dev/null && [ -f "${SCRIPT_DIR}/defectdojo-epss-patch.py" ]; then
        echo "Patching findings with EPSS scores..."
        python3 "${SCRIPT_DIR}/defectdojo-epss-patch.py" \
            "${DEFECTDOJO_URL}" "${DEFECTDOJO_API_TOKEN}" "${ENGAGEMENT_ID}" || true
    fi
