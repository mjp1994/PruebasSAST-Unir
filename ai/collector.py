...
#
# DefectDojo stores CVEs in two possible fields:
#   - cve_id          → plain string e.g. "CVE-2021-44228"
#   - vulnerability_ids → list of dicts e.g. [{"url": ".../CVE-2021-44228"}]
...
if USE_EPSS:
    ...
    cve_ids = set()
    for f in findings:
        # Field 1: cve_id (plain string)
        cve = str(f.get("cve_id") or "").strip().upper()
        if cve.startswith("CVE-"):
            cve_ids.add(cve)

        # Field 2: vulnerability_ids (list of dicts with a URL)
    ...
    # Attach EPSS to each finding (check both CVE fields)
    for f in findings:
        cve = str(f.get("cve_id") or "").strip().upper()
